<template>
  <section class="seo-text-block">
    <div class="seo-container">
      <h2>WOMEN'S GYM & WORKOUT CLOTHES</h2>
      <p>
        Workout Clothes for women designed to help you become your personal best. Functional and comfortable, we create workout clothing you'll sweat in. Since 2012, we've designed and created the gym clothes we want to wear, because training and its people are what we know best.
      </p>
      
      <h3>WORKOUT CLOTHES BUILT IN THE WEIGHT ROOM</h3>
      <p>
        Our legacy was built in the weight room. Gymshark was founded with a love for training and that passion continues into all our workout clothes today. You'll find the latest innovation in women's gym clothing and accessories to help you perform at your best and recover in style. With sophisticated seamless technology, clever contouring, and durable, quick-dry sweat-wicking fabrics, whether you're wearing 
        <NuxtLink to="/collections/womens-products">leggings</NuxtLink> or <NuxtLink to="/collections/womens-products">sports bras</NuxtLink>, 
        you will always feel comfortable, supported and confident in every workout.
      </p>
      
      <h3>SHOP WOMEN'S ACTIVEWEAR</h3>
      <p>
        We create the tools that help everyone become their personal best – no matter the sport. Our range of Women's Activewear is designed to give you the support you need to perform at your best, whether that's on the track, on the gym floor or in the studio.
      </p>
    </div>
  </section>
</template>

<style scoped>
.seo-text-block {
  background-color: #ffffff;
  padding: 4rem 0 2rem 0;
}
.seo-container {
  width: 100%;
  max-width: 1800px;
  /* Ortalamayı korumak için margin düzeni */
  margin: 0 auto; 
  padding-left: 30px;
  padding-right: 30px;
}
.seo-container h2 { font-size: 2.5rem; font-weight: 700; margin-bottom: 1rem; }
.seo-container h3 { font-size: 1.5rem; font-weight: 700; margin-top: 2rem; margin-bottom: 0.5rem; }
.seo-container p { font-size: 0.9rem; color: #555; line-height: 1.6; margin-bottom: 1rem; }
/* NuxtLink a etiketi olarak render edilir, stil aynen çalışır */
.seo-container a { color: #000; font-weight: 600; text-decoration: underline; }
</style>