...
const goToWomenCollection = () => {
  router.push('/collections/womens-products')
}
...
const fetchWomenProducts = async () => {
  try {
    loading.value = true
    const q = query(collection($db as any, 'products'), limit(4))
    ...
    console.error("Womens Sale Verisi Çekilemedi:", error)
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="womens-sale">
    <div class="container">
      
      <h2 class="section-title">WOMEN'S SALE</h2>
...
<style scoped>
.womens-sale { padding-top: 50px; padding-bottom: 3rem; width: 100%; }
.container { width: 100%; max-width: 1700px; margin: 0 auto; padding: 0 40px; }
.section-title { font-size: 1.5rem; font-weight: 700; margin-bottom: 1rem; text-transform: uppercase; text-align: left; }
